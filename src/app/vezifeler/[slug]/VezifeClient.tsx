...
export default function VezifeClient({ position, positions, sectors }: VezifeClientProps) {
  const [selectedJob, setSelectedJob] = useState<Job | null>(null);

  return (
    <ListingLayout
      title={position.name}
      subtitle={`${position.jobs.length} ${position.jobs.length === 1 ? "elan" : "elan"} tapıldı`}
      jobs={position.jobs}
      positions={positions}
      sectors={sectors}
